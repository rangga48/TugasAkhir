...
def index(request, id_jadwal):
    print(request.session['user_role_id'])
    ...
    result = []
    for x in data:
        print(x['nilai_harian_tugas'])
        x['nilai_harian_tugas'] = 0 if x['nilai_harian_tugas'] == None else x['nilai_harian_tugas']
        x['nilai_uts'] = 0 if x['nilai_uts'] == None else x['nilai_uts']
        x['nilai_uas'] = 0 if x['nilai_uas'] == None else x['nilai_uas']
        x['nilai_keterampilan'] = 0 if x['nilai_keterampilan'] == None else x['nilai_keterampilan']
        print(x)
        ...
        x['nilai_pengetahuan'] = str((int(x['nilai_harian_tugas'])+x['nilai_uts']+x['nilai_uas'])/3)
        result.append(x)
    return render(request, 'guru/penilaian/index.html', {
        "data":data,
        "id_jadwal":id_jadwal
    })

def nilai_detail(request, nis, id_jadwal):
    from io import BytesIO
    import base64
    
    data = model.getNilaiDetail({
        "c.nis":str(nis),
        "a.id_jadwal":id_jadwal
    }).getResult()
    if data == None:
    ...
    data_tugas = model.getAllTugas(str(nis), id_jadwal).getResult()
    ...
    return render(request, 'siswa/rapot/detail_nilai.html', {
        ...
        "penambahan": float(nilai_pengetahuan) + float(data['nilai_keterampilan']),
        "pembagian":(float(nilai_pengetahuan) + float(data['nilai_keterampilan']))/2,
        "graphic":graphic,
        "status_lulus":status_lulus
    })


def nilai(request, id_jadwal, nis):
    data = model.getNilai({
        "a.id_jadwal":id_jadwal,
        "b.nis":nis
    }).getResult()
    if data['nilai_uts'] != None:
        model.patchNilaiUjianSemester({
            "nilai_uts":request.POST.get("nilai_uts"),
            "nilai_uas":request.POST.get("nilai_uas"),
            "nilai_keterampilan":request.POST.get("nilai_keterampilan"),
            "nilai_harian":request.POST.get("nilai_harian")
        },{
            "id_jadwal":id_jadwal,
            "nis":nis
        })
    else:
        model.postNilaiUjianSemester({
            "id_jadwal":id_jadwal,
            "nilai_uts":request.POST.get("nilai_uts"),
            "nilai_uas":request.POST.get("nilai_uas"),
            "nilai_keterampilan":request.POST.get("nilai_keterampilan"),
    ...
    return redirect(reverse("guru:penilaian:index", kwargs={'id_jadwal':id_jadwal}))