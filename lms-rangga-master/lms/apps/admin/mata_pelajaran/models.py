...
class Model(mainModel):
    ...
    def postSiswa(self, data):
        try:
            ...
            self.cursor.execute("INSERT INTO mata_pelajaran ("+",".join(col)+") VALUES("+",".join(param)+")", val)
            self.client.commit()
            self.result = {
                "response":True,
                "action":"insert"
    ...
    def patchSiswa(self, data, where):
        try:
            ...
            val = val_up + val_wh
            self.cursor.execute("UPDATE mata_pelajaran set "+params_up.replace("and",",")+" WHERE "+params_wh, val)
            self.client.commit()
            self.result = {
                "response":True,
                "action":"update"
    ...
    def deleteSiswa(self, data):
        try:
            ...
            self.cursor.execute("DELETE FROM mata_pelajaran where "+params, val)
            self.client.commit()
            return True
        except Exception as e:
            print(e)