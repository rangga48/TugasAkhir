...
column = ["nis", "nama","jenis_kelamin","tanggal_lahir"]
class Model(mainModel):
    def getSiswa(self, data={}):
        try:
            params, values = paramsGenerator(data)
            self.cursor.execute("SELECT nis, nama, jenis_kelamin, tanggal_lahir FROM siswa where "+params, values)
    ...
    def postSiswa(self, data):
        try:
            ...
            self.cursor.execute("INSERT INTO siswa ("+",".join(col)+") VALUES("+",".join(param)+")", val)
            self.client.commit()
            self.result = {
                "response":True,
                "action":"insert"
    ...
    def patchSiswa(self, data, where):
        try:
            ...
            val = val_up + val_wh
            self.cursor.execute("UPDATE siswa set "+params_up.replace("and",",")+" WHERE "+params_wh, val)
            self.client.commit()
            self.result = {
                "response":True,
                "action":"update"
    ...
    def deleteSiswa(self, data):
        try:
            ...
            self.cursor.execute("DELETE FROM siswa where "+params, val)
            self.client.commit()
            return True
        except Exception as e:
            print(e)