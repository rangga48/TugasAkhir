...
class Model(mainModel):
    ...
    def getAllJadwal(self, data={}):
        try:
            params, values = paramsGenerator(data)
            self.cursor.execute("select c.nama as nama_mata_pelajaran, d.nama as nama_kelas, a.id_jadwal, a.id_guru, a.id_kelas, a.id_mata_pelajaran, a.waktu, a.hari from jadwal as a inner join anggota_kelas as b on a.id_kelas = b.id_kelas inner join mata_pelajaran as c on a.id_mata_pelajaran = c.id_mata_pelajaran inner join kelas as d on a.id_kelas = d.id_kelas where "+params, values)
